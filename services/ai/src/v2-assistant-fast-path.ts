...
function explicitStandaloneCustomer(transcript: string): AssistantPlan | null {
  const match = /^(?:בבקשה\s+)?(?:צור|צרי|תיצור|תצרי|הוסף|הוסיפי|תוסיף|תוסיפי)\S*\s+(?:לי\s+)?לקוח(?:ה)?\s+חדש(?:ה)?\s+בשם\s+(.+?)[.!]?$/u.exec(transcript.trim());
...
function spokenHour(text: string) {
  ...
  if (numeric) {
    ...
    const values: Array<[RegExp, number]> = [
      [/שת(?:ים|יים)\s+עשרה/u, 12], [/אחת\s+עשרה/u, 11], [/עשר/u, 10], [/תשע/u, 9],
      [/שמונה/u, 8], [/שבע/u, 7], [/שש/u, 6], [/חמש/u, 5], [/ארבע/u, 4],
      [/שלוש/u, 3], [/שת(?:ים|יים)/u, 2], [/אחת/u, 1]
    ];
...
function standaloneTask(transcript: string): AssistantPlan | null {
  const match = /^(?:בבקשה\s+)?(?:צור|צרי|תיצור|תצרי|הוסף|הוסיפי|תוסיף|תוסיפי)\S*\s+(?:לי\s+)?משימה(?:\s+חדשה)?\s+(.+?)[.!]?$/u.exec(transcript.trim());
  const title = match?.[1]?.trim();
  if (!title) return null;
  const needsInterpretation = /(?:לקוח|לקוחה|אצל|עבור|היום|מחר|מחרתיים|ביום|בבוקר|בצהריים|בערב|בלילה|בשעה|\d{1,2}:\d{2})/u.test(title);
...
function explicitCustomerActivity(transcript: string, context: unknown): AssistantPlan | null {
  ...
  const activityMarker = /\s+ו?(?:הוסף|הוסיפי|תוסיף|תוסיפי|צור|צרי|תיצור|תצרי|קבע|קבעי|תקבע|תקבעי)\s+(?:לי\s+)?(?:(?:אצל(?:ו|ה)?)\s+)?(עבודה|ביקור)(?=\s|$)/u;
  ...
  const title = /(?:^|\s)בשם\s+(.+?)(?=\s+(?:היום|מחר|מחרתיים|ביום|בשעה)|[,.;]|$)/u.exec(activityText)?.[1]?.trim()
    ?? (activityType === "job" ? "עבודה" : "ביקור");
  const time = spokenHour(activityText);
  const relativeDay = activityText.includes("מחרתיים") ? 2 : activityText.includes("מחר") ? 1 : activityText.includes("היום") ? 0 : null;
