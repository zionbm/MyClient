...
const toolInputs: Record<AssistantToolName, JsonSchema> = {
  ...
  UPDATE_CUSTOMER: strictObject({ ...customerTarget, name: nullableString, email: nullableString, generalNotes: nullableString }),
  ADD_CUSTOMER_PHONE: strictObject({ ...customerTarget, phone: stringValue, label: nullableString, isPrimary: nullableBoolean }),
  UPDATE_CUSTOMER_PHONE: strictObject({ phoneId: nullableString, entityRef: nullableReference, phone: nullableString, label: nullableString, isPrimary: nullableBoolean }),
  ...
  UPDATE_SERVICE_ADDRESS: strictObject({ addressId: nullableString, entityRef: nullableReference, addressText: nullableString, label: nullableString }),
  DELETE_SERVICE_ADDRESS: strictObject({ addressId: nullableString, entityRef: nullableReference }),
  CREATE_TASK: strictObject({ ...customerTarget, title: stringValue, description: nullableString, dueAt: nullableString, status: nullableTaskStatus }),
  UPDATE_TASK: strictObject({ taskId: nullableString, entityRef: nullableReference, customerId: nullableString, title: nullableString, description: nullableString, dueAt: nullableString, status: nullableTaskStatus }),
  ...
  UPDATE_NOTE: strictObject({ noteId: nullableString, entityRef: nullableReference, text: nullableString, status: nullableTaskStatus }),
...
function stepSchema(tool: AssistantToolName): JsonSchema {
  const kind = tool === "ASK_CLARIFICATION"
    ? "CLARIFY"
    : tool === "RESPOND"
      ? "RESPOND"
      : readTools.has(tool) ? "READ" : "WRITE";
  return strictObject({
    stepId: stringValue,
