  ...
  const plan = AssistantPlanSchema.parse({
    ...
    steps: [
      ...
      },
      {
        ...
        tool: "ADD_PAYMENT",
        dependsOn: [],
        input: { entityId: "visit-1", amount: 200 },
        confidence: 1,
        requiresExplicitConfirmation: false
      },
      {
        ...
        tool: "UPDATE_VISIT",
        dependsOn: [],
        input: { entityId: "visit-1", title: "בדיקה" },
        confidence: 1,
        requiresExplicitConfirmation: false
      }
    ]
    ...
    steps: [
      {
        ...
        tool: "FIND_CUSTOMERS",
        dependsOn: [],
        input: { query: "אורי לביא" },
        confidence: 1,
        requiresExplicitConfirmation: false
      }
    ]
  });
  const normalized = preferTranscriptCustomerName(plan, "אורי לביא לקוח בדיקה שילם על העבודה", [
  ...
  const result = AssistantPlanSchema.safeParse({
    ...
    steps: [
      ...
      },
      {
        ...
        tool: "CREATE_TASK",
        dependsOn: ["customer"],
        input: { title: "לחזור", customerRef: { stepId: "customer", outputField: "customer.id" } },
        confidence: 1,
        requiresExplicitConfirmation: false
      }
    ]
  ...
  const result = AssistantPlanSchema.safeParse({
    ...
    steps: [
      ...
      },
      {
        ...
        tool: "CREATE_TASK",
        dependsOn: [],
        input: { title: "לחזור", customerRef: { stepId: "customer", outputField: "entityId" } },
        confidence: 1,
        requiresExplicitConfirmation: false
      }
    ]
    ...
    steps: [
      {
        ...
        dependsOn: [],
        input: { date: "2026-09-02", durationMinutes: 60 },
        confidence: 1,
        requiresExplicitConfirmation: false
      },
      {
        ...
        input: { title: "לחזור", entityRef: { stepId: "availability", outputField: "entityId" } },
        confidence: 1,
        requiresExplicitConfirmation: false
      }
    ]
