...
type Note = { id: string; customerId: string; text: string; status: string; createdAt: Date; updatedAt: Date; customer?: Customer };
...
export class CoreWorkItemPresenter {
  ...
  reminderWorkItem(value: Reminder) {
    ...
    return { id: item.id, type: "reminder", title: item.title, description: item.description, customer: item.customer, dueAt: item.dueAt ?? item.createdAt, priority: item.priority, status: item.status, source: item.source, linkedEntity: { type: "reminder", id: item.id }, actions: item.status === "OPEN" ? ["call", "complete", "open"] : ["open"] };
  }
  ...
  homeVisitWorkItem(value: Visit) {
    ...
    return { id: item.id, type: "home_visit", title: item.title, description: item.notes ?? item.location, location: item.location, notes: item.notes, customer: item.customer, dueAt: item.startsAt, priority: "NORMAL", status: item.status, source: "app", linkedEntity: { type: "home_visit", id: item.id }, actions: item.status === "OPEN" ? ["navigate", "complete", "open"] : ["open"] };
  }
  appointmentWorkItem(value: Visit) {
    return { id: value.id, type: "appointment", title: value.title, description: value.notes ?? value.location, location: value.location ?? null, notes: value.notes ?? null, customer: customer(value.customer), startsAt: value.startsAt, endsAt: value.endsAt ?? null, priority: "NORMAL", status: value.status, source: "app", linkedEntity: { type: "appointment", id: value.id }, actions: value.status === "OPEN" ? ["complete", "open"] : ["open"] };
  }
  quote(value: Quote) {
  ...
  quoteWorkItem(value: Quote) {
    ...
    return { id: item.id, type: "quote", title: item.title, description: item.description, customer: item.customer, dueAt: item.dueAt, priority: "NORMAL", status: item.status, source: item.source, linkedEntity: { type: "quote", id: item.id }, actions: item.status === "OPEN" ? ["open", "edit", "mark_paid"] : ["open"] };
  }
  noteWorkItem(value: Note) {
    return { id: value.id, type: "note", title: "הערת לקוח", description: value.text, customer: customer(value.customer), dueAt: value.createdAt, priority: "NORMAL", status: value.status, source: "app", linkedEntity: { type: "note", id: value.id }, actions: value.status === "OPEN" ? ["complete", "open"] : ["open"] };
  }
}
