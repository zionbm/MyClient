...
import { UndoSchema, UpdateUserPreferencesSchema } from "@myclient/contracts";
...
function records(value: unknown) {
  return Array.isArray(value)
    ? value
        .map((item) => record(item as Prisma.JsonValue))
...
function undoSummary(mutations: ReadonlyArray<{ entityType: string; operation: string }>) {
  if (mutations.some((mutation) => mutation.operation === "MERGE"))
    return "ביטלתי את המיזוג ושחזרתי את לקוח המקור ואת כל השיוכים שהועברו.";
  if (mutations.some((mutation) => mutation.operation === "RESTORE"))
    return "ביטלתי את השחזור והחזרתי את הלקוח ואת התלויות שלו למצב המחוק הקודם.";
...
@Injectable()
export class CoreActionBatchesService {
  constructor(
    ...
    return this.idempotency.execute({
      ...
      execute: async () => {
        const result = await this.prisma.$transaction((tx) =>
          this.undoInTransaction(tx, businessId, actionBatchId, user.id)
        );
        log("info", "undo completed", { businessId, actionBatchId, mutationCount: result.revertedMutations });
        return result;
      }
    });
  }
  ...
  async undoInTransaction(tx: Prisma.TransactionClient, businessId: string, actionBatchId: string, userId: string) {
    ...
    if (!preview.eligible)
      throw new ConflictException({ code: "UNDO_BLOCKED", message: preview.reason, blockers: preview.blockers });
    const batch = await tx.actionBatch.findFirst({
      where: { id: actionBatchId, businessId },
      include: { mutations: { orderBy: { sequence: "desc" } } }
    });
    ...
    const actionBatch = await tx.actionBatch.update({
      where: { id: batch.id },
      data: { status: "UNDONE", undoneAt: new Date(), undoneByUserId: userId }
    });
  ...
  async speech(headers: RequestHeaders, businessId: string, actionBatchId: string) {
    ...
    if (!batch) throw new NotFoundException("Action batch not found");
    const summary =
      batch.status === "UNDONE" ? `הפעולה בוטלה. ${batch.finalSummary}` : (batch.spokenSummary ?? batch.finalSummary);
  ...
  async updatePreferences(headers: RequestHeaders, body: unknown) {
    const user = await this.access.requireAuthenticatedUser(headers);
    const command = UpdateUserPreferencesSchema.parse(body);
  ...
  async runRetention(headers: RequestHeaders) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const [redactedBatchTranscripts, redactedTurnTranscripts] = await Promise.all([
        tx.actionBatch.updateMany({
          where: { createdAt: { lt: transcriptCutoff }, approvedTranscript: { not: null } },
          data: { approvedTranscript: null }
        }),
        tx.assistantTurn.updateMany({
          where: { createdAt: { lt: transcriptCutoff }, approvedTranscript: { not: null } },
          data: { approvedTranscript: null }
        })
      ]);
      ...
      const [customers, tasks, jobs, visits, phones, addresses, amounts] = await Promise.all([
        ...
        tx.serviceAddress.findMany({
          where: { deletedAt: { lt: cutoff }, jobs: { none: {} }, visits: { none: {} } },
          select: { id: true }
        }),
        tx.amount.findMany({ where: { deletedAt: { lt: cutoff } }, select: { id: true } })
      ]);
      const entityIds = [...customers, ...tasks, ...jobs, ...visits, ...phones, ...addresses, ...amounts].map(
        (item) => item.id
      );
      if (entityIds.length === 0)
        return { cutoff, transcriptCutoff, redactedTranscripts, redactedMutations: 0, deletedEntities: 0 };
      const affectedBatches = await tx.actionMutation.findMany({
        where: { entityId: { in: entityIds } },
        select: { actionBatchId: true }
      });
      const redacted = await tx.actionMutation.updateMany({
        where: { entityId: { in: entityIds } },
      ...
      await tx.actionBatch.updateMany({
        ...
        data: {
          ...
          finalSummary: "פרטי הפעולה הוסרו בהתאם למדיניות השמירה.",
          spokenSummary: null
        }
      });
      ...
      const dependentJobs = customerIds.length
        ? await tx.job.findMany({ where: { customerId: { in: customerIds } }, select: { id: true } })
        : [];
      const dependentVisits = customerIds.length
        ? await tx.visit.findMany({ where: { customerId: { in: customerIds } }, select: { id: true } })
        : [];
      ...
      const dependentAmounts = await tx.amount.findMany({
        where: { OR: [{ jobId: { in: jobIds } }, { visitId: { in: visitIds } }] },
        select: { id: true }
      });
      ...
      await tx.task.deleteMany({
        where: { OR: [{ id: { in: tasks.map((item) => item.id) } }, { customerId: { in: customerIds } }] }
      });
      ...
      await tx.customerPhone.deleteMany({
        where: { OR: [{ id: { in: phones.map((item) => item.id) } }, { customerId: { in: customerIds } }] }
      });
      await tx.serviceAddress.deleteMany({
        where: { OR: [{ id: { in: addresses.map((item) => item.id) } }, { customerId: { in: customerIds } }] }
      });
      ...
      return {
        ...
        transcriptCutoff,
        redactedTranscripts,
        redactedMutations: redacted.count,
        deletedEntities: entityIds.length
      };
    });
  }

  private async preview(businessId: string, actionBatchId: string) {
    const recent = await this.prisma.actionBatch.findMany({
      where: { businessId, status: { in: ["COMPLETED", "PARTIALLY_COMPLETED", "UNDONE"] }, mutations: { some: {} } },
      orderBy: { createdAt: "desc" },
      take: 20,
      select: { id: true }
    });
    const batch = await this.prisma.actionBatch.findFirst({
      where: { id: actionBatchId, businessId },
    ...
    if (!reason) {
      ...
      const later = await this.prisma.actionMutation.findMany({
        where: {
          ...
          actionBatch: { businessId, createdAt: { gt: batch.createdAt }, undoneAt: null }
        },
        include: { actionBatch: { select: { id: true, finalSummary: true, createdAt: true } } }
      });
      blockers.push(
        ...later.map((mutation) => ({
          actionBatchId: mutation.actionBatch.id,
          summary: mutation.actionBatch.finalSummary,
          createdAt: mutation.actionBatch.createdAt
        }))
      );
    ...
    if (!reason) {
      for (const mutation of batch.mutations) {
        ...
        if (currentVersion !== after.version)
          blockers.push({ entityType: mutation.entityType, entityId: mutation.entityId, reason: "VERSION_CHANGED" });
        ...
        if (mutation.operation === "DELETE_CASCADE")
          blockers.push(...(await this.validateDeleteCascadeSnapshot(mutation.before)));
      }
  ...
  private async currentVersion(entityType: string, entityId: string) {
    if (entityType === "customer")
      return (await this.prisma.customer.findUnique({ where: { id: entityId }, select: { version: true } }))?.version;
    if (entityType === "task")
      return (await this.prisma.task.findUnique({ where: { id: entityId }, select: { version: true } }))?.version;
    if (entityType === "job")
      return (await this.prisma.job.findUnique({ where: { id: entityId }, select: { version: true } }))?.version;
    if (entityType === "visit")
      return (await this.prisma.visit.findUnique({ where: { id: entityId }, select: { version: true } }))?.version;
    if (entityType === "amount")
      return (await this.prisma.amount.findUnique({ where: { id: entityId }, select: { version: true } }))?.version;
    return undefined;
  }

  private async reverseMutation(
    ...
    if (mutation.operation === "CREATE")
      return this.softDeleteCreated(tx, mutation.entityType, mutation.entityId, actorUserId);
    ...
    if (mutation.operation === "DELETE_CASCADE")
      return this.reverseDeleteCascade(tx, mutation.entityType, mutation.entityId, before);
    if (mutation.entityType === "customer")
      return tx.customer.update({
        ...
        data: {
          ...
          normalizedName: before.normalizedName as string | null,
          generalNotes: before.generalNotes as string | null,
          deletedAt: date(before.deletedAt),
          deletedByUserId: before.deletedByUserId as string | null,
          deleteActionBatchId: before.deleteActionBatchId as string | null,
          mergedIntoCustomerId: before.mergedIntoCustomerId as string | null,
          mergedAt: date(before.mergedAt),
          mergedByUserId: before.mergedByUserId as string | null,
          version: { increment: 1 }
        }
      });
    if (mutation.entityType === "task")
      return tx.task.update({
        where: { id: mutation.entityId },
        data: {
          customerId: before.customerId as string | null,
          title: String(before.title),
          description: before.description as string | null,
          status: before.status as "OPEN" | "DONE" | "CANCELLED",
          dueAt: date(before.dueAt),
          completedAt: date(before.completedAt),
          reminderSentAt: date(before.reminderSentAt),
          source: String(before.source),
          sourceRef: before.sourceRef as string | null,
          idempotencyKey: before.idempotencyKey as string | null,
          deletedAt: date(before.deletedAt),
          deletedByUserId: before.deletedByUserId as string | null,
          deleteActionBatchId: before.deleteActionBatchId as string | null,
          version: { increment: 1 }
    ...
    if (mutation.entityType === "job" || mutation.entityType === "visit") {
      const data = {
        ...
        title: String(before.title),
        description: before.description as string | null,
        startsAt: date(before.startsAt),
        endsAt: date(before.endsAt),
        serviceAddressId: before.serviceAddressId as string | null,
        locationSnapshot: before.locationSnapshot as string | null,
        status: before.status as "OPEN" | "CLOSED" | "CANCELLED",
        executionCompletedAt: date(before.executionCompletedAt),
        executionCompletedByUserId: before.executionCompletedByUserId as string | null,
        idempotencyKey: before.idempotencyKey as string | null,
        deletedAt: date(before.deletedAt),
        deletedByUserId: before.deletedByUserId as string | null,
        deleteActionBatchId: before.deleteActionBatchId as string | null,
      ...
      return mutation.entityType === "job"
        ? tx.job.update({ where: { id: mutation.entityId }, data })
        : tx.visit.update({ where: { id: mutation.entityId }, data });
    }
    if (mutation.entityType === "amount") {
      ...
      const amount = await tx.amount.update({
        where: { id: mutation.entityId },
        data: {
          jobId: before.jobId as string | null,
          visitId: before.visitId as string | null,
          totalAmount: String(before.totalAmount),
          paidAmount: String(before.paidAmount),
          currency: String(before.currency),
          paymentStatus: before.paymentStatus as "UNPAID" | "PARTIALLY_PAID" | "PAID",
          deletedAt: date(before.deletedAt),
          deletedByUserId: before.deletedByUserId as string | null,
          deleteActionBatchId: before.deleteActionBatchId as string | null,
          version: { increment: 1 }
      ...
      await tx.amountEvent.create({
        data: {
          businessId: amount.businessId,
          amountId: amount.id,
          actorUserId,
          eventType: "UNDO",
          previousTotal: existing.totalAmount,
          nextTotal: amount.totalAmount,
          previousPaid: existing.paidAmount,
          nextPaid: amount.paidAmount,
          paidDelta: amount.paidAmount.minus(existing.paidAmount),
          source: "undo_v2"
        }
      });
      return amount;
    ...
    if (mutation.entityType === "customer_phone")
      return tx.customerPhone.update({
        ...
        data: {
          customerId: String(before.customerId),
          rawPhone: String(before.rawPhone),
          normalizedPhone: String(before.normalizedPhone),
          label: before.label as string | null,
          isPrimary: Boolean(before.isPrimary),
          deletedAt: date(before.deletedAt),
          deletedByUserId: before.deletedByUserId as string | null,
          deleteActionBatchId: before.deleteActionBatchId as string | null
        }
      });
    if (mutation.entityType === "service_address")
      return tx.serviceAddress.update({
        where: { id: mutation.entityId },
        data: {
          customerId: String(before.customerId),
          addressText: String(before.addressText),
          normalizedAddress: before.normalizedAddress as string | null,
          label: before.label as string | null,
          deletedAt: date(before.deletedAt),
          deletedByUserId: before.deletedByUserId as string | null,
          deleteActionBatchId: before.deleteActionBatchId as string | null
  ...
  private softDeleteCreated(tx: Prisma.TransactionClient, entityType: string, entityId: string, actorUserId: string) {
    ...
    if (entityType === "customer")
      return tx.customer.update({ where: { id: entityId }, data: { ...data, version: { increment: 1 } } });
    if (entityType === "task")
      return tx.task.update({ where: { id: entityId }, data: { ...data, version: { increment: 1 } } });
    if (entityType === "job")
      return tx.job.update({ where: { id: entityId }, data: { ...data, version: { increment: 1 } } });
    if (entityType === "visit")
      return tx.visit.update({ where: { id: entityId }, data: { ...data, version: { increment: 1 } } });
    if (entityType === "amount")
      return tx.amount.update({ where: { id: entityId }, data: { ...data, version: { increment: 1 } } });
    if (entityType === "customer_phone")
      return tx.customerPhone.update({ where: { id: entityId }, data: { ...data, isPrimary: false } });
  ...
  private mutationEntityIds(mutation: { entityId: string; operation: string; before: Prisma.JsonValue | null }) {
    ...
    if (mutation.operation === "RESTORE") {
      ...
      return [
        mutation.entityId,
        ...["phones", "addresses", "tasks", "jobs", "visits", "amounts"].flatMap((key) =>
          records(snapshot?.[key]).flatMap((item) => (typeof item.id === "string" ? [item.id] : []))
        )
      ];
    }
    ...
    return [
      ...
      ...["tasks", "jobs", "visits", "notes"].flatMap((key) =>
        records(snapshot?.[key]).flatMap((item) => (typeof item.id === "string" ? [item.id] : []))
      )
    ].filter(Boolean);
  }
  ...
  private async validateDeleteCascadeSnapshot(value: Prisma.JsonValue | null) {
    ...
    for (const item of records(snapshot.amounts)) {
      ...
      const current = await this.prisma.amount.findUnique({
        where: { id: item.id },
        select: { version: true, deletedAt: true }
      });
      if (!current || !current.deletedAt || current.version !== item.version + 1)
        blockers.push({ entityType: "amount", entityId: item.id, reason: "DELETE_CASCADE_STATE_CHANGED" });
    }
    return blockers;
  }

  private async reverseDeleteCascade(
    tx: Prisma.TransactionClient,
    entityType: string,
    entityId: string,
    ...
    const activity = record(snapshot?.activity as Prisma.JsonValue);
    if (!snapshot || !activity || (entityType !== "job" && entityType !== "visit"))
      throw new ConflictException("Delete cascade snapshot is missing");
    const activityData = {
      customerId: String(activity.customerId),
      title: String(activity.title),
      description: activity.description as string | null,
      startsAt: date(activity.startsAt),
      endsAt: date(activity.endsAt),
      serviceAddressId: activity.serviceAddressId as string | null,
      locationSnapshot: activity.locationSnapshot as string | null,
      status: activity.status as "OPEN" | "CLOSED" | "CANCELLED",
      executionCompletedAt: date(activity.executionCompletedAt),
      executionCompletedByUserId: activity.executionCompletedByUserId as string | null,
      idempotencyKey: activity.idempotencyKey as string | null,
      deletedAt: date(activity.deletedAt),
      deletedByUserId: activity.deletedByUserId as string | null,
      deleteActionBatchId: activity.deleteActionBatchId as string | null,
      version: { increment: 1 }
    };
    for (const amount of records(snapshot.amounts)) {
      ...
      await tx.amount.update({
        where: { id: amount.id },
        data: {
          jobId: amount.jobId as string | null,
          visitId: amount.visitId as string | null,
          totalAmount: String(amount.totalAmount),
          paidAmount: String(amount.paidAmount),
          currency: String(amount.currency),
          paymentStatus: amount.paymentStatus as "UNPAID" | "PARTIALLY_PAID" | "PAID",
          deletedAt: date(amount.deletedAt),
          deletedByUserId: amount.deletedByUserId as string | null,
          deleteActionBatchId: amount.deleteActionBatchId as string | null,
          version: { increment: 1 }
        }
      });
    }
  ...
  private async validateMergeSnapshot(value: Prisma.JsonValue | null) {
    ...
    const currentTarget = await this.prisma.customer.findUnique({
      where: { id: target.id },
      select: { version: true }
    });
    if (currentTarget?.version !== snapshot.targetAfterVersion)
      blockers.push({ entityType: "customer", entityId: target.id, reason: "VERSION_CHANGED" });
    for (const change of records(snapshot.phones)) {
      ...
      if (
        !current ||
        current.customerId !== after.customerId ||
        current.isPrimary !== after.isPrimary ||
        !sameDate(current.deletedAt, after.deletedAt)
      )
        blockers.push({ entityType: "customer_phone", entityId: after.id, reason: "MERGE_STATE_CHANGED" });
    }
    for (const change of records(snapshot.addresses)) {
      ...
      if (!current || current.customerId !== after.customerId || !sameDate(current.deletedAt, after.deletedAt))
        blockers.push({ entityType: "service_address", entityId: after.id, reason: "MERGE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.tasks)) {
      if (typeof item.id !== "string") continue;
      const current = await this.prisma.task.findUnique({
        where: { id: item.id },
        select: { customerId: true, version: true }
      });
      if (!current || current.customerId !== target.id || current.version !== item.version)
        blockers.push({ entityType: "task", entityId: item.id, reason: "MERGE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.jobs)) {
      ...
      if (!current || current.customerId !== target.id || current.version !== item.version)
        blockers.push({ entityType: "job", entityId: item.id, reason: "MERGE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.visits)) {
      ...
      if (!current || current.customerId !== target.id || current.version !== item.version)
        blockers.push({ entityType: "visit", entityId: item.id, reason: "MERGE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.notes)) {
      ...
      if (!current || current.customerId !== target.id)
        blockers.push({ entityType: "note", entityId: item.id, reason: "MERGE_STATE_CHANGED" });
    }
    return blockers;
  }
  ...
  private async validateRestoreSnapshot(value: Prisma.JsonValue | null) {
    ...
    for (const item of records(snapshot.phones)) {
      ...
      if (!current || current.deletedAt || current.deleteActionBatchId)
        blockers.push({ entityType: "customer_phone", entityId: item.id, reason: "RESTORE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.addresses)) {
      if (typeof item.id !== "string") continue;
      const current = await this.prisma.serviceAddress.findUnique({ where: { id: item.id } });
      if (!current || current.deletedAt || current.deleteActionBatchId)
        blockers.push({ entityType: "service_address", entityId: item.id, reason: "RESTORE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.tasks)) {
      if (typeof item.id !== "string") continue;
      const current = await this.prisma.task.findUnique({
        where: { id: item.id },
        select: { deletedAt: true, deleteActionBatchId: true, version: true }
      });
      if (!current || current.deletedAt || current.deleteActionBatchId || current.version !== item.version)
        blockers.push({ entityType: "task", entityId: item.id, reason: "RESTORE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.jobs)) {
      if (typeof item.id !== "string") continue;
      const current = await this.prisma.job.findUnique({
      ...
      if (!current || current.deletedAt || current.deleteActionBatchId || current.version !== item.version)
        blockers.push({ entityType: "job", entityId: item.id, reason: "RESTORE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.visits)) {
      if (typeof item.id !== "string") continue;
      const current = await this.prisma.visit.findUnique({
      ...
      if (!current || current.deletedAt || current.deleteActionBatchId || current.version !== item.version)
        blockers.push({ entityType: "visit", entityId: item.id, reason: "RESTORE_STATE_CHANGED" });
    }
    for (const item of records(snapshot.amounts)) {
      if (typeof item.id !== "string") continue;
      const current = await this.prisma.amount.findUnique({
      ...
      if (!current || current.deletedAt || current.deleteActionBatchId || current.version !== item.version)
        blockers.push({ entityType: "amount", entityId: item.id, reason: "RESTORE_STATE_CHANGED" });
    }
    return blockers;
  }
  ...
  private async reverseMerge(tx: Prisma.TransactionClient, before: Record<string, unknown>) {
    ...
    const target = record(snapshot?.target as Prisma.JsonValue);
    if (!snapshot || !source || !target || typeof source.id !== "string" || typeof target.id !== "string")
      throw new ConflictException("Merge undo snapshot is missing");
    for (const change of records(snapshot.phones)) {
      ...
      await tx.customerPhone.update({
        ...
        data: {
          ...
          isPrimary: Boolean(previous.isPrimary),
          deletedAt: date(previous.deletedAt),
          deletedByUserId: previous.deletedByUserId as string | null,
          deleteActionBatchId: previous.deleteActionBatchId as string | null
        }
      });
    }
    for (const change of records(snapshot.addresses)) {
    ...
    return tx.customer.update({
      where: { id: source.id },
      data: {
        mergedIntoCustomerId: source.mergedIntoCustomerId as string | null,
        mergedAt: date(source.mergedAt),
        mergedByUserId: source.mergedByUserId as string | null,
        version: { increment: 1 }
      }
    });
  }
  ...
  private async reverseRestore(tx: Prisma.TransactionClient, before: Record<string, unknown>) {
    ...
    const customer = record(snapshot?.customer as Prisma.JsonValue);
    if (!snapshot || !customer || typeof customer.id !== "string")
      throw new ConflictException("Restore undo snapshot is missing");
    for (const item of records(snapshot.phones)) {
      if (typeof item.id !== "string") continue;
      await tx.customerPhone.update({
        where: { id: item.id },
        data: {
          ...
          deletedAt: date(item.deletedAt),
          deletedByUserId: item.deletedByUserId as string | null,
          deleteActionBatchId: item.deleteActionBatchId as string | null
        }
      });
    }
    for (const item of records(snapshot.addresses)) {
    ...
    const restoreDeletion = (item: Record<string, unknown>) => ({
      deletedAt: date(item.deletedAt),
      deletedByUserId: item.deletedByUserId as string | null,
      deleteActionBatchId: item.deleteActionBatchId as string | null,
    ...
    for (const item of records(snapshot.tasks))
      if (typeof item.id === "string") await tx.task.update({ where: { id: item.id }, data: restoreDeletion(item) });
    for (const item of records(snapshot.jobs))
      if (typeof item.id === "string") await tx.job.update({ where: { id: item.id }, data: restoreDeletion(item) });
    for (const item of records(snapshot.visits))
      if (typeof item.id === "string") await tx.visit.update({ where: { id: item.id }, data: restoreDeletion(item) });
    for (const item of records(snapshot.amounts))
      if (typeof item.id === "string") await tx.amount.update({ where: { id: item.id }, data: restoreDeletion(item) });
    return tx.customer.update({
      where: { id: customer.id },
      data: {
        deletedAt: date(customer.deletedAt),
        deletedByUserId: customer.deletedByUserId as string | null,
        deleteActionBatchId: customer.deleteActionBatchId as string | null,
        version: { increment: 1 }
      }
    });
  }
