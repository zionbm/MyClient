...
import { IncomingCallsRepository, CustomersRepository, TasksRepository } from "./core.repositories.js";
import {
  callDisplayStatus,
  callIvrSelection,
  paginatedResponse,
...
import { normalizeIsraeliPhone } from "./normalization.js";
    ...
    return {
      calls: await Promise.all(
        page.items.map(async (call) => {
          ...
          const relatedTask = transcript?.taskId ? await this.tasks.findById(businessId, transcript.taskId) : null;
          ...
          return {
            ...
            relatedTask: relatedTask
              ? { id: relatedTask.id, status: relatedTask.status, dueAt: relatedTask.dueAt }
              : null,
            customer
          };
        })
