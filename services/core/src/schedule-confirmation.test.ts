...
test("schedule confirmation token is bound to user, schedule and conflicts", () => {
  ...
  assert.equal(
    verifyScheduleConflictToken(token, { ...expected, startsAt: "2026-09-02T08:00:00.000Z" }, secret, now),
    undefined
  );
});
...
test("moving an activity without a new end preserves its existing window duration", () => {
  ...
  assert.equal(shiftedScheduleEnd("visit", null, null, nextStart).toISOString(), "2026-09-03T12:00:00.000Z");
});
