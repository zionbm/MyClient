...
import { AssistantSessionsRepository, BusinessSettingsRepository, ActivitiesRepository } from "./core.repositories.js";
...
import {
  applyAssistantConfirmationPolicy,
  materializeStepReferences,
  preferTranscriptCustomerName,
  stepsBlockedByPlannedClarification
} from "./assistant-plan.js";
import { composeAssistantSummary } from "./assistant-reply.js";
import { assertAmountInvariant, money, nextPaidAmount, paymentStatus } from "./money.js";
import { DEFAULT_WORKING_HOURS, localDate, localDateTimeToUtc } from "./scheduling.js";
import {
...
import {
  assistantJsonValue as jsonValue,
  assistantObjectValue as objectValue,
  orderAssistantSteps as orderedSteps
...
export class CoreAssistantService {
  constructor(
    ...
    @Inject(CoreActivitiesService) private readonly activities: CoreActivitiesService,
    @Inject(ActivitiesRepository) private readonly activityRepository: ActivitiesRepository,
    @Inject(CoreActionBatchesService) private readonly actionBatches: CoreActionBatchesService,
  ...
  async createSession(headers: RequestHeaders, businessId: string, body: unknown) {
    ...
    const command = CreateAssistantSessionSchema.parse(body);
  ...
  async command(headers: RequestHeaders, businessId: string, sessionId: string, body: unknown) {
    const user = await this.access.requireBusinessAccess(headers, businessId);
    const command = AssistantCommandSchema.parse(body);
    ...
    return this.idempotency.execute({
      ...
      execute: async () => {
        ...
        const planned = await this.ai.planAssistantCommand({
        ...
        const resolvesPendingActionId =
          typeof plan.extractedFacts.resolvesPendingActionId === "string"
            ? plan.extractedFacts.resolvesPendingActionId
            : undefined;
        const confirmedPendingActionId = pendingActions.some(
          (pending) => pending.id === resolvesPendingActionId && pending.requiresExplicitConfirmation
        )
          ? resolvesPendingActionId
          : undefined;
        ...
        const execution = await this.prisma.$transaction(async (tx) => {
          ...
          for (const step of steps) {
            ...
            if (step.kind === "CLARIFY" || step.tool === "ASK_CLARIFICATION" || step.requiresExplicitConfirmation) {
              const question =
                financialProblem?.question ??
                (typeof step.input.question === "string"
                  ? step.input.question
                  : step.requiresExplicitConfirmation
                    ? this.confirmationQuestion({ ...step, input: pendingInput })
                    : "נדרש מידע נוסף");
              const pending = await tx.aiPendingAction.create({
                data: {
                  ...
                  payload: jsonValue({
                    tool: step.tool,
                    input: pendingInput,
                    confirmationRequiredAfterInput: financialProblem !== undefined
                  }),
            ...
            if (output.waiting) {
              ...
              const createCustomerSuggestion =
                typeof pendingOptions.createCustomerName === "string"
              ...
              const pending = await tx.aiPendingAction.create({
                data: {
                  ...
                  missingFields: Array.isArray(pendingOptions.missingFields)
                    ? (pendingOptions.missingFields as string[])
                    : [],
                  actionBatchId: actionBatch.id,
                  ...
                  requiresExplicitConfirmation:
                    step.requiresExplicitConfirmation || pendingOptions.requiresExplicitConfirmation === true
                }
              });
          ...
          const status = waiting ? (completedCount > 0 ? "PARTIALLY_COMPLETED" : "WAITING") : "COMPLETED";
          ...
          const rejectedPendingActionId =
            typeof plan.extractedFacts.rejectsPendingActionId === "string"
          ...
          if (rejectedPendingActionId) {
            ...
            if (declined) {
              ...
              if (declined.actionBatchStepId) {
                await tx.actionBatchStep.update({
                  where: { id: declined.actionBatchStepId },
          ...
          return {
            ...
            voiceResult: {
              ...
              items: receiptSteps.map((step) => ({
                ...
                title: this.voiceItemTitle(step.tool, step.status, step.displayPayload ?? step.entity),
        ...
        log("info", "assistant command executed", {
          businessId,
  ...
  async listPending(headers: RequestHeaders, businessId: string, query: unknown) {
    await this.access.requireBusinessAccess(headers, businessId);
    const command = PendingActionsQuerySchema.parse(query);
    ...
    return {
      ...
      totalCount: await this.prisma.aiPendingAction.count({ where: { businessId, status: "PENDING" } })
    };
  }
    ...
    return this.idempotency.execute({
      ...
      execute: async () => {
        const existing = await this.prisma.aiPendingAction.findFirst({
          where: { id: pendingActionId, businessId, status: "PENDING" }
        });
        ...
        const action = await this.prisma.aiPendingAction.update({
          ...
          data: { payload: command.payload ? jsonValue(command.payload) : undefined, question: command.question }
        });
        return { action };
      }
    });
  ...
  async rejectPending(headers: RequestHeaders, businessId: string, pendingActionId: string) {
    ...
    return this.idempotency.execute({
      ...
      execute: async () => {
        ...
        const rejected = await this.prisma.aiPendingAction.update({
          where: { id: action.id },
          data: { status: "REJECTED", resolvedAt: new Date(), resolvedByUserId: user.id }
        });
        log("info", "pending action resolved", {
          businessId,
          outcome: "REJECTED",
          ageMs: Date.now() - action.createdAt.getTime(),
          actionType: action.actionType
        });
        if (action.actionBatchStepId)
          await this.prisma.actionBatchStep.update({
            where: { id: action.actionBatchStepId },
            data: { status: "REJECTED" }
          });
        if (action.actionBatchId)
          await this.prisma.actionBatch.update({
            where: { id: action.actionBatchId },
            data: { status: "PARTIALLY_COMPLETED", finalSummary: "הפעולה הממתינה נדחתה." }
          });
  ...
  async resolvePending(headers: RequestHeaders, businessId: string, pendingActionId: string, body: unknown) {
    const user = await this.access.requireBusinessAccess(headers, businessId);
    const command = ResolvePendingActionSchema.parse(body);
    return this.idempotency.execute({
      businessId,
      userId: user.id,
      ...
      execute: async () =>
        this.prisma.$transaction(async (tx) => {
          const pending = await tx.aiPendingAction.findFirst({
            where: { id: pendingActionId, businessId, status: "PENDING" }
          });
          if (!pending?.actionBatchId || !pending.actionBatchStepId)
            throw new NotFoundException("Pending action not found");
          if (pending.requiresExplicitConfirmation && command.confirmed !== true)
          ...
          for (const step of allSteps) {
            if (step.status === "COMPLETED" && step.output)
              outputs.set(step.stepKey, step.output as Record<string, unknown>);
          }
          ...
          let selectedOutput: Record<string, unknown> = {
            entityId: command.selectedEntityId,
            ...(command.payload ?? {})
          };
          const createCustomerName =
            typeof command.payload?.createCustomerName === "string" ? command.payload.createCustomerName.trim() : "";
          if (plannedCurrentStep.tool === "FIND_CUSTOMERS" && createCustomerName) {
            const createCommand = CreateCustomerSchema.parse({ name: createCustomerName });
          ...
          if (plannedCurrentStep.kind === "WRITE") {
            ...
            const confirmedStep = {
              ...
              input: {
                ...plannedCurrentStep.input,
                ...objectValue(storedPendingPayload.input),
                ...confirmationOverrides,
                ...(command.payload ?? {}),
                ...(command.selectedEntityId ? { entityId: command.selectedEntityId } : {})
              }
            };
            if (storedPendingPayload.confirmationRequiredAfterInput === true && !pending.requiresExplicitConfirmation) {
              ...
              await tx.aiPendingAction.update({
                ...
                data: {
                  payload: jsonValue({
                    ...storedPendingPayload,
                    input: confirmedStep.input,
                    confirmationRequiredAfterInput: false
                  }),
                  question,
                  missingFields: [],
                  requiresExplicitConfirmation: true
            ...
            const executed = await this.executeBasicWrite(tx, {
              businessId,
              userId: user.id,
              key: requiredIdempotencyKey(headers),
              headers,
            ...
            if (executed.waiting) {
              ...
              await tx.aiPendingAction.update({
                ...
                data: {
                  payload: jsonValue({
                    tool: plannedCurrentStep.tool,
                    input: confirmedStep.input,
                    confirmationOverrides: refreshed.confirmationOverrides
                  }),
              ...
              log("info", "pending action refreshed", {
                businessId,
                actionType: pending.actionType,
                reason: "STILL_MISSING_INPUT"
              });
            ...
            if (executed.entityType && executed.entityId && executed.entity) {
              ...
              await tx.actionMutation.create({
                data: {
                  ...
                  operation: executed.operation ?? "CREATE",
                  before: executed.before ? jsonValue(executed.before) : undefined,
                  after: jsonValue(executed.entity)
                }
              });
            }
          }
          await tx.actionBatchStep.update({
            ...
            data: { status: "COMPLETED", output: jsonValue(selectedOutput) }
          });
          ...
          log("info", "pending action resolved", {
            businessId,
            outcome: "COMPLETED",
            ageMs: Date.now() - pending.createdAt.getTime(),
            actionType: pending.actionType
          });
          for (const step of orderedSteps(plan.steps)) {
            ...
            if (
              ...
              !step.dependsOn.every((dependency) => outputs.has(dependency))
            )
              continue;
            const output = await this.executeBasicWrite(tx, {
              businessId,
          ...
          const remaining = await tx.actionBatchStep.count({
            where: { actionBatchId: batch.id, status: { in: ["WAITING", "BLOCKED"] } }
          });
          const summary =
            remaining === 0 ? "השלמתי את הפעולה הממתינה." : "שמרתי את התשובה, אך עדיין חסר מידע לפעולה נוספת.";
          await tx.actionBatch.update({
            where: { id: batch.id },
            data: { status: remaining === 0 ? "COMPLETED" : "PARTIALLY_COMPLETED", finalSummary: summary }
          });
    ...
    for (const [id, expected] of Object.entries(entityVersions)) {
      ...
      if (actual !== expected)
        throw new ConflictException({
          ...
          message: "The selected entity changed; review the action again"
        });
    }
  }
  ...
  private confirmationQuestion(step: AssistantPlanStep) {
    ...
    if (["SET_ACTIVITY_AMOUNT", "ADD_PAYMENT", "SET_PAID_TOTAL", "SETTLE_BALANCE"].includes(step.tool)) {
      const amount =
        typeof step.input.amount === "number"
          ? ` של ${step.input.amount} ₪`
          : typeof step.input.totalAmount === "number"
            ? ` ל-${step.input.totalAmount} ₪`
            : "";
  ...
  private async financialConfirmationProblem(
    ...
    const nextTotal = step.tool === "SET_ACTIVITY_AMOUNT" ? money(Number(step.input.totalAmount)) : previousTotal;
    const mode =
      step.tool === "ADD_PAYMENT" ? "ADD" : step.tool === "SET_PAID_TOTAL" ? "SET_PAID_TOTAL" : "SETTLE_BALANCE";
    const nextPaid =
      step.tool === "SET_ACTIVITY_AMOUNT"
        ? typeof step.input.paidAmount === "number"
          ? money(step.input.paidAmount)
          : previousPaid
        : nextPaidAmount(
            mode,
            previousPaid,
            nextTotal,
            typeof step.input.amount === "number" ? step.input.amount : undefined
          );
    try {
  ...
  private async executeBasicWrite(
    ...
    if (input.step.tool === "FIND_CUSTOMERS") {
      const query =
        typeof input.step.input.query === "string"
          ? input.step.input.query
          : typeof input.step.input.name === "string"
            ? input.step.input.name
            : "";
      ...
      return {
        ...
        entityVersion: candidates[0]!.version,
        message: `מצאתי את ${candidates[0]!.name}.`
      };
    }
    if (["FIND_TASKS", "FIND_JOBS", "FIND_VISITS"].includes(input.step.tool)) {
      const customerId =
        typeof input.step.input.customerId === "string"
      ...
      if (input.emptyReadIsResult) {
        const noun =
          input.step.tool === "FIND_TASKS" ? "משימות" : input.step.tool === "FIND_JOBS" ? "עבודות" : "ביקורים";
        return {
      ...
      if (candidates.length !== 1) {
        return {
          ...
          question:
            candidates.length === 0
              ? "לא מצאתי פעילות יחידה שמתאימה לבקשה."
              : "מצאתי כמה פעילויות מתאימות. באיזו לבחור?",
      ...
      return {
        ...
        entityId: candidates[0]!.id,
        entityVersion: candidates[0]!.version,
        message: `מצאתי את ${candidates[0]!.title}.`
      };
    }
    if (input.step.tool === "GET_AVAILABILITY") {
      ...
      return {
        result,
        message:
          times.length > 0 ? `החלונות הפנויים הראשונים הם ${times.join(", ")}.` : "לא מצאתי חלון פנוי ביום המבוקש."
      };
    }
    if (input.step.tool === "GET_SCHEDULE") {
      ...
      return {
        result: { items },
        message: items.length > 0 ? `הפעילות הבאה היא ${items[0]!.title}.` : "לא מצאתי פעילות מתוזמנת בטווח המבוקש."
      };
    }
    if (input.step.tool === "GET_CUSTOMER_TIMELINE") {
      ...
      if (!customerId)
        return { waiting: true as const, question: "על איזה לקוח להציג פעילות?", missingFields: ["customerId"] };
      const [tasks, jobs, visits, notes] = await Promise.all([
        tx.task.findMany({
          where: { businessId: input.businessId, customerId, deletedAt: null },
      ...
      return {
        result: { tasks, jobs, visits, notes },
        message: count === 0 ? "לא מצאתי פעילות קודמת ללקוח." : `מצאתי ${count} פריטים בציר הזמן של הלקוח.`
      };
    }
    if (input.step.tool === "GET_ACTIVITY_AMOUNT") {
      ...
      const amount = entityId
        ? await tx.amount.findFirst({
            where: { businessId: input.businessId, deletedAt: null, OR: [{ jobId: entityId }, { visitId: entityId }] }
          })
        : null;
      ...
      return {
        result: { amount },
        entityId: amount.id,
        message: `הסכום הוא ${amount.totalAmount.toString()} שקלים, ומתוכם שולמו ${amount.paidAmount.toString()} שקלים.`
      };
    }
    if (input.step.tool === "GET_PAYMENT_SUMMARY") {
      const from =
        typeof input.step.input.from === "string"
          ? new Date(input.step.input.from)
          : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      const to = typeof input.step.input.to === "string" ? new Date(input.step.input.to) : new Date();
      const events = await tx.amountEvent.findMany({
        where: {
          businessId: input.businessId,
      ...
      return {
        result: { totalPaid: total, eventCount: events.length },
        message: `בתקופה המבוקשת התקבלו ${total.toString()} שקלים.`
      };
    }
    if (input.step.tool === "GET_OPEN_BALANCES") {
      const amounts = await tx.amount.findMany({
        where: {
          businessId: input.businessId,
          deletedAt: null,
      ...
      return {
        result: { totalBalance: balance, count: amounts.length },
        message: `היתרה הפתוחה היא ${balance.toString()} שקלים ב-${amounts.length} פעילויות.`
      };
    }
    if (input.step.tool === "GET_TODAY_OVERVIEW") {
      ...
      const [tasks, jobs, visits] = await Promise.all([
        tx.task.findMany({
          where: { businessId: input.businessId, status: "OPEN", deletedAt: null, dueAt: { lt: endsAt } },
          orderBy: { dueAt: "asc" },
          take: 50
        }),
        tx.job.findMany({
    ...
    if (input.step.tool === "RESPOND") {
      return {
        ...
        message: typeof input.step.input.text === "string" ? input.step.input.text : "הבקשה נבדקה."
      };
    }
    if (input.step.tool === "CREATE_CUSTOMER") {
      const command = CreateCustomerSchema.parse(input.step.input);
      const customer = await tx.customer.create({
        data: {
          businessId: input.businessId,
          name: command.name,
      ...
      return {
        ...
        entityId: customer.id,
        entity: customer,
        message: `נוצר הלקוח ${customer.name}. אפשר להוסיף לו טלפון או כתובת בהודעה הבאה.`
      };
    }
    if (input.step.tool === "CREATE_TASK") {
      const command = CreateTaskSchema.parse(input.step.input);
      const customerId =
        command.customerId ?? this.resolveCustomerReference(input.step.input.customerRef, input.outputs);
      if (
        customerId &&
      ...
      const customer = task.customerId
        ? await tx.customer.findFirst({ where: { id: task.customerId, businessId: input.businessId, deletedAt: null } })
        : null;
      return {
        entityType: "task",
        entityId: task.id,
        entity: task,
        displayPayload: { ...task, customerName: customer?.name },
        message: `נפתחה המשימה "${task.title}"${dueText}.`
      };
    }
    if (input.step.tool === "UPDATE_TASK") {
      ...
      const existing = taskId
        ? await tx.task.findFirst({ where: { id: taskId, businessId: input.businessId, deletedAt: null } })
        : null;
      if (!existing)
        return { waiting: true as const, question: "לא מצאתי את המשימה לעדכון.", missingFields: ["taskId"] };
      const hasCustomerId = Object.prototype.hasOwnProperty.call(input.step.input, "customerId");
      const nextCustomerId =
        hasCustomerId && (typeof input.step.input.customerId === "string" || input.step.input.customerId === null)
          ? input.step.input.customerId
          : undefined;
      if (
        typeof nextCustomerId === "string" &&
        !(await tx.customer.findFirst({
      ...
      const entity = await tx.task.update({
        where: { id: existing.id },
        data: {
          ...
          dueAt: typeof input.step.input.dueAt === "string" ? new Date(input.step.input.dueAt) : undefined,
          version: { increment: 1 }
        }
      });
      const customer = entity.customerId
        ? await tx.customer.findFirst({
            where: { id: entity.customerId, businessId: input.businessId, deletedAt: null }
          })
        : null;
      return {
        entityType: "task",
        entityId: entity.id,
        entity,
        displayPayload: { ...entity, customerName: customer?.name },
        before: existing,
        operation: "UPDATE",
        message: `המשימה "${entity.title}" עודכנה.`
      };
    }
    if (input.step.tool === "CREATE_NOTE") {
      const customerId = this.resolveEntityId(input.step.input.customerId, input.step.input.customerRef, input.outputs);
      const command = CreateNoteSchema.parse({ text: input.step.input.text, status: input.step.input.status });
      const customer = customerId
        ...
        : null;
      if (!customer)
        return { waiting: true as const, question: "צריך לבחור לקוח עבור ההערה.", missingFields: ["customerId"] };
      const entity = await tx.note.create({
        data: { businessId: input.businessId, customerId: customer.id, text: command.text, status: command.status }
      });
      return {
        entityType: "note",
        entityId: entity.id,
        entity,
        displayPayload: { ...entity, customerId: customer.id, customerName: customer.name },
        message: `נוספה הערה ללקוח ${customer.name}.`
      };
    }
    if (input.step.tool === "UPDATE_NOTE") {
      ...
      const existing = noteId
        ? await tx.note.findFirst({ where: { id: noteId, businessId: input.businessId, deletedAt: null } })
        : null;
      if (!existing)
        return { waiting: true as const, question: "לא מצאתי את ההערה לעדכון.", missingFields: ["noteId"] };
      const command = UpdateNoteSchema.parse({ text: input.step.input.text, status: input.step.input.status });
      ...
      return {
        ...
        entityId: entity.id,
        entity,
        displayPayload: { ...entity, customerId: entity.customerId, customerName: customer?.name },
        before: existing,
        operation: "UPDATE"
      };
    }
    if (input.step.tool === "UPDATE_CUSTOMER") {
      ...
      if (!existing)
        return { waiting: true as const, question: "לא מצאתי את הלקוח לעדכון.", missingFields: ["customerId"] };
      ...
      return {
        ...
        before: existing,
        operation: "UPDATE",
        message: `פרטי הלקוח ${entity.name} עודכנו.`
      };
    }
    if (input.step.tool === "ADD_CUSTOMER_PHONE") {
      ...
      if (!customerId || !phone)
        return {
          waiting: true as const,
          question: "צריך לקוח ומספר טלפון.",
          missingFields: [!customerId ? "customerId" : "phone"]
        };
      const customer = await tx.customer.findFirst({
        where: {
          id: customerId,
      ...
      if (!customer)
        return { waiting: true as const, question: "לא מצאתי את הלקוח להוספת הטלפון.", missingFields: ["customerId"] };
      const normalizedPhone = normalizeIsraeliPhone(phone);
      if (!normalizedPhone)
        return { waiting: true as const, question: "מספר הטלפון אינו תקין.", missingFields: ["phone"] };
      ...
      if (duplicate)
        return {
          waiting: true as const,
          question: `מספר הטלפון כבר משויך ללקוח ${duplicate.customer.name}.`,
          missingFields: ["phone"]
        };
      const activeCount = await tx.customerPhone.count({
        where: { businessId: input.businessId, customerId, deletedAt: null }
      });
      const entity = await tx.customerPhone.create({
        data: {
          businessId: input.businessId,
          customerId,
          rawPhone: phone,
          normalizedPhone,
          label: typeof input.step.input.label === "string" ? input.step.input.label : undefined,
    ...
    if (input.step.tool === "UPDATE_CUSTOMER_PHONE" || input.step.tool === "DELETE_CUSTOMER_PHONE") {
      ...
      const existing = phoneId
        ? await tx.customerPhone.findFirst({ where: { id: phoneId, businessId: input.businessId, deletedAt: null } })
        : null;
      if (!existing)
        return { waiting: true as const, question: "לא מצאתי את מספר הטלפון לעדכון.", missingFields: ["phoneId"] };
      ...
      const entity = await tx.customerPhone.update({
        where: { id: existing.id },
        data: deleting
          ? { deletedAt: new Date(), deletedByUserId: input.userId, isPrimary: false }
          : {
              rawPhone,
              normalizedPhone: normalizedPhone ?? undefined,
              label: typeof input.step.input.label === "string" ? input.step.input.label : undefined,
              isPrimary: typeof input.step.input.isPrimary === "boolean" ? input.step.input.isPrimary : undefined
            }
      });
    ...
    if (input.step.tool === "ADD_SERVICE_ADDRESS") {
      ...
      if (!customerId || !addressText)
        return {
          waiting: true as const,
          question: "צריך לקוח וכתובת שירות.",
          missingFields: [!customerId ? "customerId" : "addressText"]
        };
      const customer = await tx.customer.findFirst({
        where: { id: customerId, businessId: input.businessId, deletedAt: null, mergedIntoCustomerId: null },
        select: { id: true }
      ...
      if (!customer)
        return { waiting: true as const, question: "הלקוח שנבחר אינו זמין עוד.", missingFields: ["customerId"] };
      const entity = await tx.serviceAddress.create({
        data: {
          businessId: input.businessId,
          customerId,
          addressText,
          normalizedAddress: normalizeServiceAddress(addressText),
          label: typeof input.step.input.label === "string" ? input.step.input.label : undefined
        }
      });
    ...
    if (input.step.tool === "UPDATE_SERVICE_ADDRESS" || input.step.tool === "DELETE_SERVICE_ADDRESS") {
      ...
      const existing = addressId
        ? await tx.serviceAddress.findFirst({ where: { id: addressId, businessId: input.businessId, deletedAt: null } })
        : null;
      if (!existing)
        return { waiting: true as const, question: "לא מצאתי את כתובת השירות לעדכון.", missingFields: ["addressId"] };
      const deleting = input.step.tool === "DELETE_SERVICE_ADDRESS";
      const addressText =
        typeof input.step.input.addressText === "string" ? input.step.input.addressText.trim() : undefined;
      const entity = await tx.serviceAddress.update({
        where: { id: existing.id },
        data: deleting
          ? { deletedAt: new Date(), deletedByUserId: input.userId }
          : {
              addressText,
              normalizedAddress: addressText ? normalizeServiceAddress(addressText) : undefined,
              label: typeof input.step.input.label === "string" ? input.step.input.label : undefined
            }
      });
      return {
    ...
    if (input.step.tool === "RESTORE_CUSTOMER") {
      ...
      const existing = customerId
        ? await tx.customer.findFirst({
            where: { id: customerId, businessId: input.businessId, deletedAt: { not: null } }
          })
        : null;
      if (!existing)
        return { waiting: true as const, question: "לא מצאתי לקוח שניתן לשחזר.", missingFields: ["customerId"] };
      ...
      if (existing.deleteActionBatchId) {
        ...
        [
          restoreSnapshot.phones,
          restoreSnapshot.addresses,
          restoreSnapshot.tasks,
          restoreSnapshot.jobs,
          restoreSnapshot.visits,
          restoreSnapshot.amounts
        ] = await Promise.all([
        ...
        await Promise.all([
          tx.customerPhone.updateMany({
            where,
            data: { deletedAt: null, deletedByUserId: null, deleteActionBatchId: null }
          }),
          tx.serviceAddress.updateMany({
            where,
            data: { deletedAt: null, deletedByUserId: null, deleteActionBatchId: null }
          }),
    ...
    if (input.step.tool === "MERGE_CUSTOMERS") {
      const sourceId = this.resolveEntityId(
        input.step.input.sourceCustomerId,
        input.step.input.sourceCustomerRef,
        input.outputs
      );
      const targetId = this.resolveEntityId(
        input.step.input.targetCustomerId,
        input.step.input.targetCustomerRef,
        input.outputs
      );
      if (!sourceId || !targetId || sourceId === targetId)
        return {
          waiting: true as const,
          question: "צריך לבחור לקוח מקור ולקוח יעד שונים.",
          missingFields: ["sourceCustomerId", "targetCustomerId"]
        };
      const [source, target] = await Promise.all([
        tx.customer.findFirst({
          where: { id: sourceId, businessId: input.businessId, deletedAt: null, mergedIntoCustomerId: null }
        }),
        tx.customer.findFirst({
          where: { id: targetId, businessId: input.businessId, deletedAt: null, mergedIntoCustomerId: null }
        })
      ]);
      if (!source || !target)
        return { waiting: true as const, question: "אחד הלקוחות למיזוג לא נמצא.", missingFields: ["customers"] };
      ...
      for (const phone of sourcePhones) {
        ...
        const after = await tx.customerPhone.update({
          where: { id: phone.id },
          data: duplicate
            ? { deletedAt: new Date(), deletedByUserId: input.userId, isPrimary: false }
      ...
      for (const address of sourceAddresses) {
        const duplicate = address.normalizedAddress
          ...
          : null;
        const after = await tx.serviceAddress.update({
          where: { id: address.id },
          data: duplicate ? { deletedAt: new Date(), deletedByUserId: input.userId } : { customerId: target.id }
        });
    ...
    if (["COMPLETE_TASK", "CANCEL_TASK", "REOPEN_TASK", "DELETE_TASK"].includes(input.step.tool)) {
      ...
      if (!existing)
        return { waiting: true as const, question: "לא מצאתי את המשימה לעדכון.", missingFields: ["taskId"] };
      const data =
        input.step.tool === "DELETE_TASK"
          ? { deletedAt: new Date(), deletedByUserId: input.userId, version: { increment: 1 } }
          : {
              status:
                input.step.tool === "COMPLETE_TASK"
                  ? ("DONE" as const)
                  : input.step.tool === "CANCEL_TASK"
                    ? ("CANCELLED" as const)
                    : ("OPEN" as const),
              completedAt:
                input.step.tool === "COMPLETE_TASK" ? new Date() : input.step.tool === "REOPEN_TASK" ? null : undefined,
              version: { increment: 1 }
            };
      ...
      return {
        ...
        entity,
        before: existing,
        operation: input.step.tool === "DELETE_TASK" ? "DELETE" : "UPDATE",
        message
      };
    }
    if (input.step.tool === "CREATE_JOB" || input.step.tool === "CREATE_VISIT") {
      ...
      if (!customerId || !title)
        return {
          waiting: true as const,
          question: "צריך לקוח וכותרת לפעילות.",
          missingFields: [!customerId ? "customerId" : "title"]
        };
      const startsAt = typeof input.step.input.startsAt === "string" ? new Date(input.step.input.startsAt) : undefined;
      const explicitEndsAt =
        typeof input.step.input.endsAt === "string" ? new Date(input.step.input.endsAt) : undefined;
      ...
      if (
        ...
        return {
          waiting: true as const,
          question: "המועד אינו תקין. צריך שעת התחלה ושעת סיום מאוחרת ממנה.",
      ...
      const result = await this.activityRepository.createInTransaction(tx, {
        ...
        endsAt,
        serviceAddressId:
          typeof input.step.input.serviceAddressId === "string" ? input.step.input.serviceAddressId : undefined,
        locationSnapshot:
          typeof input.step.input.locationSnapshot === "string" ? input.step.input.locationSnapshot : undefined,
        idempotencyKey: `${input.key}:${input.step.stepId}`,
        allowScheduleConflict: false,
        approvedConflictFingerprint: startsAt
          ? this.assistantApprovedConflictFingerprint(input.step.input.scheduleConflictToken, {
              businessId: input.businessId,
              userId: input.userId,
              operation: "CREATE",
              kind,
      ...
      if ("missingLink" in result)
        return {
          waiting: true as const,
          question: "הלקוח או כתובת השירות אינם זמינים עוד.",
          missingFields: ["customerOrAddress"]
        };
      if (!("entity" in result))
        return this.scheduleConflictPending(
          input.businessId,
          input.userId,
          "CREATE",
          null,
      ...
      return {
        ...
        entity,
        displayPayload: { ...entity, customerName: customer?.name },
        message: `נ${kind === "job" ? "פתחה עבודה" : "פתח ביקור"} "${entity.title}"${windowText}.`,
        warnings: await this.activities.scheduleWarnings(input.businessId, startsAt, endsAt, kind)
      };
    }
    if (input.step.tool === "UPDATE_JOB" || input.step.tool === "UPDATE_VISIT") {
      ...
      const existing = entityId
        ? kind === "job"
          ? await tx.job.findFirst({ where: { id: entityId, businessId: input.businessId, deletedAt: null } })
          : await tx.visit.findFirst({ where: { id: entityId, businessId: input.businessId, deletedAt: null } })
        : null;
      if (!existing)
        return { waiting: true as const, question: "לא מצאתי את הפעילות לעדכון.", missingFields: ["entityId"] };
      const startsAt = typeof input.step.input.startsAt === "string" ? new Date(input.step.input.startsAt) : undefined;
      const explicitEndsAt =
        typeof input.step.input.endsAt === "string" ? new Date(input.step.input.endsAt) : undefined;
      const endsAt =
        startsAt && explicitEndsAt === undefined
      ...
      if ("invalidSchedule" in result)
        return {
          waiting: true as const,
          question: "שעת הסיום חייבת להיות אחרי שעת ההתחלה.",
          missingFields: ["schedule"]
        };
      if ("missingLink" in result)
        return {
      ...
      if ("notFound" in result)
        return {
          waiting: true as const,
          question: "הפעילות השתנתה מאז הבדיקה. צריך לבדוק שוב.",
          missingFields: ["entityVersion"]
        };
      if (!("entity" in result))
        return this.scheduleConflictPending(
          input.businessId,
      ...
      return {
        ...
        before: existing,
        operation: "UPDATE",
        message: `${kind === "job" ? "העבודה" : "הביקור"} "${entity.title}" עודכ${kind === "job" ? "נה" : "ן"}.`,
        warnings: await this.activities.scheduleWarnings(
          input.businessId,
    ...
    if (
      ...
      const existing = entityId
        ? kind === "job"
          ? await tx.job.findFirst({
              where: { id: entityId, businessId: input.businessId, deletedAt: null },
              include: { amounts: { where: { deletedAt: null } } }
            })
      ...
      const data = isDelete
        ? { deletedAt, deletedByUserId: input.userId, version: { increment: 1 } }
        : isCancel
          ? { status: "CANCELLED" as const, version: { increment: 1 } }
          : isReopen
            ? {
                status: "OPEN" as const,
                executionCompletedAt: null,
                executionCompletedByUserId: null,
                version: { increment: 1 }
              }
            : {
                executionCompletedAt: completedAt,
                executionCompletedByUserId: input.userId,
                status: paid || noCharge ? ("CLOSED" as const) : ("OPEN" as const),
                version: { increment: 1 }
              };
      if (isDelete) {
        await tx.amount.updateMany({
          where: {
            businessId: input.businessId,
            deletedAt: null,
            ...(kind === "job" ? { jobId: existing.id } : { visitId: existing.id })
          },
          data: { deletedAt, deletedByUserId: input.userId, version: { increment: 1 } }
        });
      }
      const entity =
        kind === "job"
          ? await tx.job.update({ where: { id: existing.id }, data })
          : await tx.visit.update({ where: { id: existing.id }, data });
      const { amounts, ...activityBefore } = existing;
      return {
        entityType: kind,
        entityId: entity.id,
        entity,
        before: isDelete ? { deleteCascade: { activity: activityBefore, amounts } } : existing,
        operation: isDelete ? "DELETE_CASCADE" : "UPDATE"
      };
    }
    if (["SET_ACTIVITY_AMOUNT", "ADD_PAYMENT", "SET_PAID_TOTAL", "SETTLE_BALANCE"].includes(input.step.tool)) {
      const entityId = this.resolveEntityId(input.step.input.entityId, input.step.input.entityRef, input.outputs);
      if (!entityId)
        return { waiting: true as const, question: "לאיזו פעילות לעדכן את הסכום?", missingFields: ["entityId"] };
      const existing = await tx.amount.findFirst({
        where: { businessId: input.businessId, deletedAt: null, OR: [{ jobId: entityId }, { visitId: entityId }] }
      });
      if (input.step.tool !== "SET_ACTIVITY_AMOUNT" && !existing)
        return {
          waiting: true as const,
          question: "צריך להגדיר סכום כולל לפני עדכון תשלום.",
      ...
      const visit = job
        ...
        : await tx.visit.findFirst({ where: { id: entityId, businessId: input.businessId, deletedAt: null } });
      if (!job && !visit)
        return { waiting: true as const, question: "לא מצאתי את הפעילות.", missingFields: ["entityId"] };
      const previousTotal = existing?.totalAmount ?? money(0);
      const previousPaid = existing?.paidAmount ?? money(0);
      const nextTotal =
        input.step.tool === "SET_ACTIVITY_AMOUNT" ? money(Number(input.step.input.totalAmount)) : previousTotal;
      const mode =
        input.step.tool === "ADD_PAYMENT"
          ? "ADD"
          : input.step.tool === "SET_PAID_TOTAL"
            ? "SET_PAID_TOTAL"
            : "SETTLE_BALANCE";
      const nextPaid =
        input.step.tool === "SET_ACTIVITY_AMOUNT"
          ? typeof input.step.input.paidAmount === "number"
            ? money(input.step.input.paidAmount)
            : previousPaid
          : nextPaidAmount(
              mode,
              previousPaid,
              nextTotal,
              typeof input.step.input.amount === "number" ? input.step.input.amount : undefined
            );
      try {
        assertAmountInvariant(nextTotal, nextPaid);
      } catch {
        return {
          waiting: true as const,
          question: "הסכום ששולם אינו יכול להיות גבוה מהסכום הכולל. איך לתקן?",
          missingFields: ["totalAmountOrPaidAmount"]
        };
      }
      ...
      const entity = existing
        ...
        : await tx.amount.create({
            data: {
              businessId: input.businessId,
              ...(job ? { jobId: entityId } : { visitId: entityId }),
      ...
      const eventType =
        input.step.tool === "SET_ACTIVITY_AMOUNT"
          ? existing
            ? ("CHANGE_TOTAL" as const)
            : ("CREATE" as const)
          : input.step.tool === "ADD_PAYMENT"
            ? ("ADD_PAYMENT" as const)
            : input.step.tool === "SET_PAID_TOTAL"
              ? ("SET_PAID_TOTAL" as const)
              : ("SETTLE_BALANCE" as const);
      await tx.amountEvent.create({
        data: {
          businessId: input.businessId,
          amountId: entity.id,
      ...
      if (activity.status !== "CANCELLED") {
        ...
        if (job) await tx.job.update({ where: { id: job.id }, data: { status: nextStatus } });
        else await tx.visit.update({ where: { id: visit!.id }, data: { status: nextStatus } });
      }
      return {
        entityType: "amount",
        entityId: entity.id,
    ...
    if (input.step.tool === "UNDO_ACTION_BATCH") {
      const requestedId =
        typeof input.step.input.actionBatchId === "string" ? input.step.input.actionBatchId : undefined;
      ...
      if (!target)
        return {
          waiting: true as const,
          question: "לא מצאתי פעולה אחרונה שניתן לבטל.",
          missingFields: ["actionBatchId"]
        };
  ...
  private async completeSatisfiedCallTask(
    ...
    const customerIds = [
      ...new Set(
        activitySteps
          .map((step) => step.input.customerId)
  ...
  private voiceItemTitle(tool: unknown, status: unknown, entity: unknown) {
    ...
    return tool === "CREATE_CUSTOMER"
      ? `לקוח חדש${customerName ? `: ${customerName}` : ""}`
      : tool === "ADD_CUSTOMER_PHONE"
        ? `טלפון נוסף${typeof value.customerName === "string" ? `: ${value.customerName}` : ""}`
        : tool === "CREATE_JOB" || tool === "UPDATE_JOB"
          ? (entityTitle ?? "עבודה")
          : tool === "CREATE_VISIT" || tool === "UPDATE_VISIT"
            ? (entityTitle ?? "ביקור")
            : tool === "CREATE_TASK" || tool === "UPDATE_TASK"
              ? (entityTitle ?? "משימה")
              : tool === "CREATE_NOTE" || tool === "UPDATE_NOTE"
                ? "הערה ללקוח"
                : tool === "COMPLETE_TASK"
                  ? `משימה הושלמה${entityTitle ? `: ${entityTitle}` : ""}`
                  : "הפעולה הושלמה";
  }

  private voiceItemKind(tool: unknown) {
    return tool === "CREATE_CUSTOMER" || tool === "UPDATE_CUSTOMER" || tool === "ADD_CUSTOMER_PHONE"
      ? "customer"
      : typeof tool === "string" && tool.includes("TASK")
        ? "reminder"
        : typeof tool === "string" && tool.includes("JOB")
          ? "job"
          : typeof tool === "string" && tool.includes("VISIT")
            ? "home_visit"
            : typeof tool === "string" && tool.includes("NOTE")
              ? "note"
              : "action";
  }

  private assistantApprovedConflictFingerprint(
    token: unknown,
    input: {
      businessId: string;
      userId: string;
    ...
    return verifyScheduleConflictToken(
      token,
      {
        businessId: input.businessId,
        userId: input.userId,
