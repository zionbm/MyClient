...
import { normalizeCustomerName } from "./normalization.js";
...
export function materializeStepReferences(step: AssistantPlanStep, outputs: Map<string, Record<string, unknown>>) {
  ...
  const entityIdField = step.tool.includes("TASK")
    ? "taskId"
    : step.tool.includes("NOTE")
      ? "noteId"
      : step.tool.includes("CUSTOMER_PHONE")
        ? "phoneId"
        : step.tool.includes("SERVICE_ADDRESS")
          ? "addressId"
          : "entityId";
...
export function applyAssistantConfirmationPolicy(plan: AssistantPlan, confirmedPendingActionId?: string) {
  const resolvesConfirmedPending =
    confirmedPendingActionId !== undefined && plan.extractedFacts.resolvesPendingActionId === confirmedPendingActionId;
  return AssistantPlanSchema.parse({
    ...plan,
    steps: plan.steps.map((step) => ({
      ...step,
      requiresExplicitConfirmation:
        step.requiresExplicitConfirmation || (!resolvesConfirmedPending && assistantToolRequiresConfirmation(step.tool))
    }))
  });
}
...
export function preferTranscriptCustomerName(
  ...
  const mentioned = customers
    .map((customer) => ({
      ...customer,
      normalizedName: customer.normalizedName ?? normalizeCustomerName(customer.name)
    }))
  ...
  if (
    mentioned[1]?.normalizedName.length === longest.normalizedName.length &&
    mentioned[1].normalizedName !== longest.normalizedName
  )
    return plan;
  return AssistantPlanSchema.parse({
    ...plan,
    steps: plan.steps.map((step) =>
      step.tool === "FIND_CUSTOMERS" ? { ...step, input: { ...step.input, query: longest.name } } : step
    )
  });
}
