...
import { normalizeIsraeliPhone, normalizeServiceAddress } from "./normalization.js";
...
export class CoreSearchService {
  ...
  async search(headers: RequestHeaders, businessId: string, query: unknown) {
                  ...
                  OR: [
                    ...
                    { email: contains },
                    {
                      customerPhones: {
                        some: {
                          deletedAt: null,
                          OR: [{ rawPhone: contains }, ...(normalizedPhone ? [{ normalizedPhone }] : [])]
                        }
                      }
                    },
                    {
                      serviceAddresses: {
                        some: {
                          deletedAt: null,
                          OR: [{ addressText: contains }, { normalizedAddress: { contains: normalizedAddress } }]
                        }
                      }
                    }
                  ]
      ...
      wants("tasks")
        ? this.prisma.task.findMany({
            where: {
              ...
              deletedAt: null,
              status: taskStatus,
              AND: [cursorWhere, { OR: [{ title: contains }, { description: contains }] }]
            },
      ...
      wants("jobs")
        ? this.prisma.job.findMany({
            where: {
              ...
              AND: [
                cursorWhere,
                { OR: [{ title: contains }, { description: contains }, { customer: { name: contains } }] }
              ]
            },
            include: { customer: true },
            take,
    ...
    return {
      ...
      pageInfo: {
        hasMore,
        nextCursor: hasMore && last ? encodePageCursor({ id: last.id, createdAt: last.updatedAt }) : null
      }
    };
  }
}
