...
import { CreateJobSchema, UpdateActivitySchema, WorkingHoursSchema } from "@myclient/contracts";
import {
...
test("free slots exclude overlaps and use deterministic 30-minute starts", () => {
  ...
  assert.deepEqual(
    ...
    ["2026-09-06T05:00:00.000Z", "2026-09-06T07:00:00.000Z"]
  );
});

test("the suggestion interval does not restrict an explicitly requested minute", () => {
  const explicitQuarterHour = CreateJobSchema.safeParse({
...
test("working-hours settings reject malformed or reversed open windows", () => {
  ...
  assert.equal(
    WorkingHoursSchema.safeParse({ saturday: { open: "00:00", close: "00:00", closed: true } }).success,
    true
  );
});
...
test("public activity contracts accept confirmation tokens and reject the legacy override flag", () => {
  ...
  assert.equal(CreateJobSchema.safeParse({ ...create, scheduleConflictToken: "signed-token" }).success, true);
  assert.equal(CreateJobSchema.safeParse({ ...create, allowScheduleConflict: true }).success, false);
  assert.equal(
    UpdateActivitySchema.safeParse({ startsAt: "2026-09-02T10:00:00+03:00", allowScheduleConflict: true }).success,
    false
  );
});
