...
function effectiveEnd(activity: { startsAt: Date | null; endsAt: Date | null }, kind: ActivityKind) {
...
export class ActivitiesRepository {
  ...
  async list(kind: ActivityKind, businessId: string, pagination: ActivityListInput) {
    const args = {
      where: {
        businessId,
        deletedAt: null,
        ...
        executionCompletedAt:
          pagination.executed === undefined ? undefined : pagination.executed ? { not: null } : null,
  ...
  findById(kind: ActivityKind, businessId: string, entityId: string, tx?: Prisma.TransactionClient) {
    const args = {
  ...
  async createInTransaction(
    ...
    if (
      conflicts.length > 0 &&
      !input.allowScheduleConflict &&
      !sameConflictFingerprint(input.approvedConflictFingerprint, scheduleConflictFingerprint(conflicts))
    )
      return { conflicts };
    const {
      kind: _kind,
      allowScheduleConflict: _allowScheduleConflict,
      approvedConflictFingerprint: _approvedConflictFingerprint,
      ...activityInput
    } = input;
    const data = { ...activityInput, endsAt, locationSnapshot };
    const entity = input.kind === "job" ? await tx.job.create({ data }) : await tx.visit.create({ data });
    return { entity, conflicts };
  }

  async update(
    input: Partial<ActivityWrite> & {
      kind: ActivityKind;
      entityId: string;
      businessId: string;
      version?: number;
  ...
  async updateInTransaction(
    ...
    const data = {
      ...
      locationSnapshot:
        input.locationSnapshot ?? (input.serviceAddressId !== undefined ? linked.addressText : undefined),
      status: input.status,
  ...
  async schedule(businessId: string, from: Date, to: Date) {
    ...
    return [
      ...jobs
        .filter((item) => effectiveEnd(item, "job")! > from)
        .map((item) => ({ ...item, kind: "job" as const, effectiveEndsAt: effectiveEnd(item, "job") })),
      ...visits
        .filter((item) => effectiveEnd(item, "visit")! > from)
        .map((item) => ({ ...item, kind: "visit" as const, effectiveEndsAt: effectiveEnd(item, "visit") }))
  ...
  private async validateLinks(
    ...
    const address = await tx.serviceAddress.findFirst({
      where: { id: serviceAddressId, businessId, customerId, deletedAt: null }
    });
  ...
  private async conflictsInTransaction(
    ...
    return [
      ...jobs
        .filter((item) => effectiveEnd(item, "job")! > startsAt)
        .map((item) => ({ ...item, kind: "job", effectiveEndsAt: effectiveEnd(item, "job") })),
      ...visits
        .filter((item) => effectiveEnd(item, "visit")! > startsAt)
        .map((item) => ({ ...item, kind: "visit", effectiveEndsAt: effectiveEnd(item, "visit") }))
    ];
  }
}
