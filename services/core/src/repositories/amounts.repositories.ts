...
import { activityStatusAfterAmount, assertAmountInvariant, money, nextPaidAmount, paymentStatus } from "../money.js";
import { inRepositoryTransaction } from "./repository.shared.js";
import type { ActivityKind } from "./activities.repositories.js";
...
@Injectable()
export class AmountsRepository {
  constructor(@Inject(PrismaService) private readonly prisma: PrismaService) {}

  find(kind: ActivityKind, businessId: string, entityId: string) {
  ...
  async set(
    ...
    return inRepositoryTransaction(this.prisma, transaction, async (tx) => {
      ...
      const eventType: AmountEventType = !existing
        ? "CREATE"
        : input.change.paidAmount !== undefined
          ? "CORRECTION"
          : "CHANGE_TOTAL";
  ...
  async payment(
    ...
    return inRepositoryTransaction(this.prisma, transaction, async (tx) => {
      ...
      const eventType: AmountEventType =
        input.mode === "ADD" ? "ADD_PAYMENT" : input.mode === "SET_PAID_TOTAL" ? "SET_PAID_TOTAL" : "SETTLE_BALANCE";
      await this.event(
        tx,
        input,
        amount.id,
  ...
  async paymentReport(businessId: string, from: Date, to: Date) {
    const events = await this.prisma.amountEvent.findMany({
      ...
      include: {
        amount: { include: { job: { include: { customer: true } }, visit: { include: { customer: true } } } }
      },
  ...
  async openBalances(businessId: string) {
    ...
    const totalBalance = amounts.reduce(
      (sum, amount) => sum.plus(amount.totalAmount.minus(amount.paidAmount)),
      money(0)
    );
    return { totalBalance, amounts };
  }

  private activity(tx: Prisma.TransactionClient, kind: ActivityKind, businessId: string, entityId: string) {
    return kind === "job"
