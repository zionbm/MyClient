...
type TaskListInput = PaginationInput & {
  state?: "OPEN" | "CLOSED";
  customerId?: string;
  dueBefore?: Date;
...
export class CustomersRepository {
  ...
  async softDelete(
    input: { businessId: string; customerId: string; deletedByUserId: string },
  ...
  async merge(
    input: { businessId: string; sourceCustomerId: string; targetCustomerId: string; actorUserId: string },
    transaction?: Prisma.TransactionClient
  ) {
    ...
    return inRepositoryTransaction(this.prisma, transaction, async (tx) => {
      ...
      for (const phone of sourcePhones) {
        ...
        if (duplicate) {
          await tx.customerPhone.update({
            where: { id: phone.id },
            data: { deletedAt: new Date(), deletedByUserId: input.actorUserId, isPrimary: false }
          });
        } else {
      ...
      for (const address of sourceAddresses) {
        ...
        await tx.serviceAddress.update({
          where: { id: address.id },
          data: duplicate ? { deletedAt: new Date(), deletedByUserId: input.actorUserId } : { customerId: target.id }
        });
      }
      await Promise.all([
        tx.task.updateMany({
...
@Injectable()
export class CustomerPhonesRepository {
  ...
  async softDelete(
    input: { businessId: string; customerId: string; phoneId: string; deletedByUserId: string },
    transaction?: Prisma.TransactionClient
  ) {
    return inRepositoryTransaction(this.prisma, transaction, async (tx) => {
      const existing = await tx.customerPhone.findFirst({
...
@Injectable()
export class ServiceAddressesRepository {
  constructor(@Inject(PrismaService) private readonly prisma: PrismaService) {}
  ...
  async softDelete(
    input: { businessId: string; customerId: string; addressId: string; deletedByUserId: string },
    tx?: Prisma.TransactionClient
  ) {
    const db = tx ?? this.prisma;
    const existing = await db.serviceAddress.findFirst({
...
export class TasksRepository {
  ...
  async list(businessId: string, pagination: TaskListInput) {
    await this.businesses.requireBusiness(businessId);
  ...
  async softDelete(
    input: { businessId: string; taskId: string; deletedByUserId: string },
    tx?: Prisma.TransactionClient
  ) {
    const db = tx ?? this.prisma;
    const updated = await db.task.updateMany({
...
export class NotesRepository {
  ...
  async update(
    ...
    return updated.count === 1
      ? db.note.findFirst({
          where: { id: input.noteId, businessId: input.businessId, customerId: input.customerId, deletedAt: null }
        })
      : null;
  }
  ...
  async softDelete(businessId: string, customerId: string, noteId: string, tx?: Prisma.TransactionClient) {
    ...
    return updated.count === 1 ? db.note.findUnique({ where: { id: noteId } }) : null;
  }
}
