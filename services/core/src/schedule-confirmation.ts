...
export function issueScheduleConflictToken(
  input: Omit<ScheduleConflictClaims, "version" | "expiresAt"> & { now?: Date; ttlMs?: number },
  secret: string
) {
...
export function verifyScheduleConflictToken(
  token: string,
  expected: Omit<ScheduleConflictClaims, "version" | "conflictFingerprint" | "expiresAt">,
  secret: string,
  now = new Date()
) {
  ...
  if (claims.version !== 1 || claims.expiresAt <= now.getTime() || !Array.isArray(claims.conflictFingerprint))
    return undefined;
  if (
    claims.businessId !== expected.businessId ||
    ...
    claims.startsAt !== expected.startsAt ||
    claims.endsAt !== expected.endsAt
  )
    return undefined;
