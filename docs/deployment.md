...
`INTERNAL_API_SECRET`, `DB_PASSWORD`, and `DATABASE_URL` are populated by the script if needed. `OPENAI_API_KEY` must be populated manually before deploying `ai` or `voice` with real OpenAI providers.
