...
const customerResult = request(
  "POST",
  `${core}/v2/businesses/${businessId}/customers`,
  {
    name: "לקוח אינטגרציה",
    email: `customer-${suffix}@example.com`,
    generalNotes: "לקוח שנוצר בבדיקת המוצר"
  },
...
expectStatus(
  request(
    "POST",
    `${core}/v2/businesses/${businessId}/customers/${customerId}/phones`,
    {
      phone: "+972502222222",
      isPrimary: true
    },
    authHeaders(`phone-${suffix}`)
  ),
  201,
  "create customer phone"
);

const addressResult = request(
  "POST",
  `${core}/v2/businesses/${businessId}/customers/${customerId}/addresses`,
  {
    label: "בית",
...
const taskResult = request(
  "POST",
  `${core}/v2/businesses/${businessId}/tasks`,
  {
    customerId,
    title: "לחזור ללקוח",
...
expectStatus(
  request(
    "POST",
    `${core}/v2/businesses/${businessId}/tasks/${taskId}/complete`,
    {},
    authHeaders(`task-complete-${suffix}`)
  ),
  201,
  "complete task"
);
...
const jobResult = request(
  "POST",
  `${core}/v2/businesses/${businessId}/jobs`,
  {
    customerId,
...
expectStatus(
  request(
    "PUT",
    `${core}/v2/businesses/${businessId}/jobs/${jobId}/amount`,
    {
      totalAmount: 500,
      currency: "ILS"
    },
    authHeaders(`amount-${suffix}`)
  ),
  200,
  "set job amount"
);
expectStatus(
  request(
    "POST",
    `${core}/v2/businesses/${businessId}/jobs/${jobId}/amount/payments`,
    {
      mode: "ADD",
      amount: 200
    },
    authHeaders(`payment-${suffix}`)
  ),
  201,
  "record payment"
);
...
expectStatus(
  request(
    "GET",
    `${core}/v2/businesses/${businessId}/schedule?from=${encodeURIComponent(startsAt.toISOString())}&to=${encodeURIComponent(endsAt.toISOString())}`,
    undefined,
    authHeaders()
  ),
  200,
...
expectStatus(
  request("POST", `${core}/internal/tasks/due`, { limit: 100 }, { "x-internal-secret": "dev-internal-secret" }),
  201,
  "process due tasks"
);
